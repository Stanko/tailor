...
function toFixed(n, decimals = 1) {
    return +n.toFixed(decimals);
}
function setPosition($el, left, top, width, height) {
    $el.style.left = `${left}px`;
    $el.style.top = `${top}px`;
    $el.style.width = `${width}px`;
    $el.style.height = `${height}px`;
}
function setHorizontalRuler($el, xStart, yStart, xEnd, fixValue = 0, addNumber = false) {
    if (xStart > xEnd) {
        const tmp = xEnd;
        xEnd = xStart;
        xStart = tmp;
...
function setVerticalRuler($el, xStart, yStart, yEnd, fixValue = 0, addNumber = false) {
    if (yStart > yEnd) {
        const tmp = yEnd;
        yEnd = yStart;
        yStart = tmp;
...
class Tailor {
    constructor() {
        ...
        // Singleton
        if (window.__tailor_instance) {
            return window.__tailor_instance;
        }
        document.body.append(this.$tailor);
        window.addEventListener("keydown", this.handleKeyDown);
        window.addEventListener("keyup", this.handleKeyUp);
        window.__tailor_instance = this;
        console.log("%c T ", "background-color: #0b99ff; color: white; line-height: 17px; display: inline-block;", `Tailor initialized`);
    }
    // ----- CONTROLS ----- //
    enable() {
    ...
    destroy() {
        this.disable();
        this.$tailor.remove();
        delete window.__tailor_instance;
    ...
    highlightElement($el) {
        ...
        setPosition($margin, -margin.left, -margin.top, box.width + margin.inline, box.height + margin.block);
        let marginHTML = "";
        let paddingHTML = "";
    ...
    measureDistance($current, $measureTo) {
        const from = getRect($current);
        const to = getRect($measureTo);
        const { $xRuler, $xRuler2, $yRuler, $yRuler2, $xRulerHelper, $xRulerHelper2, $yRulerHelper, $yRulerHelper2, } = this;
        if ($measureTo) {
